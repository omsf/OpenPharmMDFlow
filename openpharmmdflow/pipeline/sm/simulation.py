...
def create_simulation(
    ...
    simulation_chunk_step_size = getattr(simulate_config, 'simulation_chunk_step_size', save_data_frequency_steps)
...
def setup_trajectory_reporters(simulation, simulate_config, output_directory):
    ...
    print(f"   🔸 Thermodynamic data: {data_file} (every {save_data_frequency_steps} steps)")
    
    # 3. Checkpoint Reporter (For resuming simulations)
    checkpoint_file = os.path.join(output_directory, "checkpoint.chk")
    checkpoint_multiplier = getattr(simulate_config, 'checkpoint_frequency_multiplier', 10)
    ...
    try:
        ...
        if os.path.exists(h5_file):
            print(f"   ⚠️ HDF5 trajectory exists, skipping to avoid file lock: {h5_file}")
        else:
...
def run_simulation(
    ...
    try:
        ...
        pe_initial = minimized_state.getPotentialEnergy().value_in_unit(openmm.unit.kilojoules_per_mole)
    ...
    chunk_size = min(simulation_chunk_step_size, 50000)  # Use new parameter for chunk size
    ...
    for start_step in range(0, n_steps, chunk_size):
        ...
        try:
            ...
            if start_step % simulation_chunk_step_size == 0 or end_step == n_steps:
                ...
                volume_nm3 = np.linalg.det(box_vectors.value_in_unit(openmm.unit.nanometer))
                ...
                temp_k_current = simulation.integrator.getTemperature().value_in_unit(openmm.unit.kelvin)
                
                print(f"{step_num:8d}, {time_ps:10.2f}, {volume_nm3:10.3f}, {temp_k_current:12.1f}, {pe:12.1f}")
            ...
            emergency_checkpoint = os.path.join(output_dir, f"emergency_checkpoint_{start_step}.chk")
...
def resume_simulation(
    simulate_config,
    simulation: openmm.app.Simulation,
    checkpoint_file: Union[str, None] = None
):
