...
from pyxtal.io import read_cif
...
def prep_molecule(workingdir, molecule, count):
    xyzfile = os.path.join(workingdir, f"molec-{count}.xyz")
    molfile = os.path.join(workingdir, f"molec-{count}.mol")
    with open(xyzfile, "w") as f:
        f.write(f"{len(molecule)}\n\n")
        for a in molecule:
            ID = a.as_dict()["name"]
            xyz = a.as_dict()["xyz"]
            f.write(f"{ID:s}\t{xyz[0]:f}\t{xyz[1]:f}\t{xyz[2]:f}\n")
...
def reorder(molec, template):
    ...
    with open(critic_inp, "w") as f:
        f.write(f"MOLREORDER {template:s} {molec:d}.xyz WRITE {new:s}")
    Command = f"critic2 {critic_inp}"
    Command = Command.split()
    subprocess.call(Command, stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT)
        ...
        if atom.GetNumRadicalElectrons() > 0:
            ...
            for bond in atom.GetBonds():
                if (
                    ...
                    or bond.GetBeginAtom().GetSymbol() == "N"
                ):
...
def residue_setup(tagged_residues, images, ss_dim, file=None, forcefield="GAFF"):
    ...
    parm_residue = [
        molfile for molfile in os.listdir(parmtarget) if molfile.endswith(".mol")
    ]
    rd_mol = []
...
def structure_setup(topology, generator, structure, **kwargs):
    ...
    context_coords = [
        structure.sites[i].coords / 10 for i in range(0, len(structure.sites))
    ]
    context_lattice = structure.lattice.matrix / 10
    system.setDefaultPeriodicBoxVectors(
        context_lattice[0], context_lattice[1], context_lattice[2]
    )
...
class ProcessCell:
    ...
    def standard_cell(self):
        numbers = [
            self.structure.sites[x].specie.number
            for x in range(len(self.structure.sites))
        ]
        ...
        self.standard_structure = Structure(
            self.dataset["std_lattice"],
            self.dataset["std_types"],
            self.dataset["std_positions"],
        )
    ...
    def ordered_crystal(self):
        ...
        unique_num_atoms = {len(m) for m in molecs}  # Chemically unique
        unique_molecules = []
        count = 0
        ...
        asym_molecules = center_molecules(
            asym_molecules, self.standard_structure.lattice.matrix
        )
        xtal_abc = []
        xtal_species = []
        ID = 0
        ...
        for m in asym_molecules:
            ...
            m.to(os.path.join("critic2", f"{ID}.xyz"))
            reordered_molecule = reorder(
                ID, os.path.join(parmtarget, f"molec-{parmtag}.xyz")
            )
            ...
            for t in self.translators:
                for o in self.operations:
                    for c in range(len(species)):
                        ...
                        abc = np.matmul(
                            np.linalg.inv(self.standard_structure.lattice.matrix).T, xyz
                        )
                        abc = np.matmul(o[0], abc) + o[1] + t
                        abc = np.matmul(e_T.T, abc)
                        xtal_abc.append(abc)
        self.standard_structure.make_supercell(self.ss)
        ordered_cell = Structure(
            self.standard_structure.lattice.matrix, xtal_species, xtal_abc
        )
...
if __name__ == "__main__":
    ...
    for f in os.listdir("cif"):
        ...
        # Default SMIRNOFF
        topology, generator = residue_setup(
            reordered, images, ss_dim, forcefield="smirnoff"
        )
        [system, context_coords, context_lattice, omm_top] = structure_setup(
            topology, generator, ordered, custom=False
        )
